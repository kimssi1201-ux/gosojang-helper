...
function buildFactSection(payload, evidenceItems) {
  ...
  const typeFacts = buildTypeSpecificFactText(payload, evidenceItems);
...
function buildTypeSpecificFactText(payload, evidenceItems) {
  const date = valueOr(payload.incidentDate, "[사건 일시]");
  const place = valueOr(payload.incidentPlace, "[사건 장소]");
  const damage = valueOr(payload.damage, "[피해금액 또는 피해내용]");
  const story = sentenceOrPlaceholder(payload.story, "[피고소인의 말과 행동을 시간순으로 기재]");
  ...
  return templates[payload.caseTypeId] || {
    ...
    legalPoint: `이 사건은 ${valueOr(payload.caseTypeName, "[범죄유형]")} 혐의와 관련된 사실로 정리됩니다.`,
  };
}
...
buildPurpose = function (payload) {
  ...
  return `고소인은 피고소인을 ${valueOr(payload.caseTypeName, "관련 범죄")} 혐의로 고소하오니, 철저히 수사하여 처벌하여 주시기 바랍니다.`;
};
...
buildFactSection = function (payload, evidenceList) {
  ...
  return [
    ...
    "",
    "라. 피해 결과",
    buildDamageParagraph(payload, elements),
    "",
    "마. 증거와의 연결",
    buildEvidenceConnection(payload, evidenceList, elements),
  ].join("\n");
};

function buildAccusedSpecificLine(payload) {
...
var mapElementsByCrimeType = function (crimeType) {
  const base = { evidenceGuide: "증거자료는 해당 사실을 입증하는 취지와 연결해 정리해야 합니다.", damagePoint: "" };
  const byType = {
...
function getQuestionAnswersFromPayload(payload) {
  return Array.isArray(payload.questionAnswers) ? payload.questionAnswers : [];
}
...
function buildEvidenceList(payload) {
  const titles = splitEvidence(payload.evidence);
...
buildReasonSection = function (payload, evidenceList) {
  const intent = valueOr(payload.punishmentIntent, "고소인은 피고소인의 처벌을 원합니다.");
  return [
    `고소인은 위 범죄사실과 같이 ${valueOr(payload.caseTypeName, "형사사건")} 관련 피해를 입었습니다.`,
...
buildCautions = function (payload, evidenceList) {
  const missing = checkMissingFields(payload);
  const lines = [];
  if (missing.length) {
    lines.push("[보완 필요]");
...
var checkMissingFields = function (payload) {
  const missing = [];
  ...
  const common = [
    [payload.complainant, "고소인 성명이 없습니다."],
    [payload.complainantAddress, "고소인 주소가 없습니다."],
    [payload.complainantPhone, "고소인 연락처가 없습니다."],
    [accusedKnown, "피고소인 성명, 연락처, 주소, 계정, 계좌번호 등 특정 단서가 없습니다."],
    [payload.incidentDate, "사건 일시가 구체적이지 않습니다. 가능한 날짜 범위라도 입력하세요."],
    [payload.incidentPlace, "사건 장소가 없습니다. 장소는 관할 수사기관과 범죄사실 특정에 도움이 됩니다."],
    [payload.story || payload.actDetail, "피고소인의 구체적 행위가 부족합니다. 실제로 한 말, 보낸 메시지, 받은 돈, 폭행 방법 등을 적으세요."],
    [payload.damage || payload.damageDetail, "피해 내용이 없습니다. 피해금액, 치료기간, 수리비, 영업손실 등을 적으세요."],
    [payload.evidence || payload.evidenceDescription, "증거자료가 없습니다. 증거명과 입증하려는 사실을 함께 적으세요."],
  ];
...
findMissingInfo = function (payload) {
  const missing = checkMissingFields(payload);
  return missing.length ? missing : ["현재 입력 기준으로 핵심 항목은 채워져 있습니다."];
};

function typeSpecificMissing(payload) {
  const text = [payload.story, payload.actDetail, payload.damage, payload.evidence, payload.evidenceDescription, ...getQuestionAnswersFromPayload(payload).map((item) => `${item.question} ${item.answer}`)].join(" ");
  const groups = {
...
getCaseTypeRequirements = function (payload) {
  const byType = {
  ...
  return byType[payload.caseTypeId] || byType.other;
};

buildHelpfulChecklist = function (payload, aiMissingInfo = []) {
  const baseMissing = Array.isArray(aiMissingInfo) && aiMissingInfo.length ? aiMissingInfo : findMissingInfo(payload);
  const items = baseMissing.map((item) => item.startsWith("현재 ") ? item : `보완 필요: ${item}`);
  items.push(`${valueOr(payload.caseTypeName, "선택한 범죄유형")} 핵심 확인: ${getCaseTypeRequirements(payload).join(" / ")}`);
